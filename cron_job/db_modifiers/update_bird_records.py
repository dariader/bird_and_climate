from cron_job.utils import open_connection, format_insert_keys, format_insert_values
...
class UpdateBirdRecords:
    ...
    def calculate_n_days_back(self):
        difference = datetime.now().date() - self.latest_date
    ...
    def insert_into_db(self):
        if self.dif_days == 0 or len(self.new_data) == 0:
            return
        cur = self.cnx.cursor()
        # eh we need to insert only columns we already have in db
        # so we need to specify which keys we need to pass
        cur.execute("SELECT column_name FROM information_schema.columns where table_name = 'cy';")
        ...
        for i in self.new_data:
            ...
            i_keys = format_insert_keys(keys_overlap)
            i_values = format_insert_values([prepared_dict.get(key) for key in keys_overlap], fix_null=True)  # rewrite
